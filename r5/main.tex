...
Let $L$ be a $K_{r+1}$-free graph on $n$ vertices that is not
$r$-partite, and let $R=\comp L$ be admissible.
\begin{enumerate}[label=\textup{(\roman*)}]
\item If $(r,n)=(2,10)$, then $e(L)\leq20$.
\item If $(r,n)=(3,15)$, equality in \eqref{eq:KP} is impossible.
\end{enumerate}
\end{lemma}
...
$A$ belonging to $P$, and the three vertices in the parity class labeled
$A\setminus P$
...
\]
because $d_M(a)+d_M(a')$ counts every edge incident with $a$ or $a'$ once
and counts $aa'$ a second time,
...
\begin{enumerate}[label=\textup{(\roman*)}]
\item There is a partition $V(R)=C\mathbin{\dot\cup}V(R_0)$ with
$|C|=5$, $R[C]=K_5$, and exactly one edge between $C$ and $V(R_0)$, where
$\comp{R_0}$ is the balanced two-fold blow-up of $C_5$.
