#!/usr/bin/env python3
"""Semantically verify exact rational duals for strict m=62 core-shell pairs.

This verifier independently reconstructs the 2 cores, the 1,147 compatible
...
solver. It proves only the 2,142 pairs represented by strict exact duals.
"""
...
EXPECTED_CORES = 2
EXPECTED_SHELLS = 1147
EXPECTED_PAIRS = EXPECTED_CORES * EXPECTED_SHELLS
EXPECTED_CERTIFICATES = 2142
EXPECTED_UNCERTIFIED = EXPECTED_PAIRS - EXPECTED_CERTIFICATES
EXPECTED_CORE_CATALOG_SHA256 = (
    "d86a3b06e752d86a723cafdbefc5030b8f993794de5f03e34193f9495953fc65"
)
EXPECTED_SHELL_CATALOG_SHA256 = (
    "5529c3fbabe7ba673cf52baca23dcbac6b6a09a31e97b8db5fa3c5ef7b9b0132"
)
EXPECTED_UNCERTIFIED_PAIR_SHA256 = (
    "f5a0febbfb028eb99aa4201df31232ec46ded7f1c4299011d753d29b031532fb"
)
EXPECTED_SEMANTIC_SHA256 = (
    "e6e3487f1ea47e8fb254de021102e6139cbb297c92d1fd739137d8c41454ad45"
)
EXPECTED_DATA_SHA256 = (
    "d80b2e95446ca6771c6c64591c43addc256e49c4bf099e8e09e7c648ad013796"
)
NONCLAIM = (
    "The 152 uncertified pairs remain open in this package; it does not "
    "exclude m=62, prove fixed r=9, or solve Erdos Problem 617."
)
...
def generate_cores(geng: Path) -> tuple[GraphCase, ...]:
    command = [str(geng), "-q", "-t", "-D8", "16", "62:62"]
    cases = []
...
def core_demand_profile(core: nx.Graph) -> tuple[int, int]:
    if not nx.is_bipartite(core):
        fail("m=62 core is not bipartite")
    ...
    if sorted(map(len, sides)) != [8, 8]:
        fail("m=62 core does not have an 8+8 bipartition")
    if len(independent_eights(core)) != 2:
        fail("m=62 core does not have exactly two independent eight-sets")
    demand_sums = tuple(
...
def parse_header(raw: Any) -> None:
    ...
    expected = {
        "schema": "erdos617-r9-m62-duals-v1",
...
def main() -> None:
    ...
    print(f"core_structure=K8,8-minus-2 demand_profile={dict(demand_profile)}")
