#!/usr/bin/env python3
"""Semantically verify exact rational duals for strict m=61 core-shell pairs.

This verifier independently reconstructs the 4 cores, the 393 compatible
...
solver. It proves only the 1,461 pairs represented by strict exact duals.
"""
...
EXPECTED_CERTIFICATES = 1461
EXPECTED_UNCERTIFIED = EXPECTED_PAIRS - EXPECTED_CERTIFICATES
EXPECTED_CORE_CATALOG_SHA256 = (
    "80c5dd280c51a2b48e7cd7996292832aae426f4e5afb2d3ee4244b917881b958"
)
EXPECTED_SHELL_CATALOG_SHA256 = (
    "5abd46a2e45b0077717b991849af13dda42ecbd314f061c1b205dc7ec2d0b2e1"
)
EXPECTED_UNCERTIFIED_PAIR_SHA256 = (
    "0001c726de2073aced1bdeb9b95a5cb48b8387ce0012ab41479c9404c51d7ff2"
)
EXPECTED_SEMANTIC_SHA256 = (
    "352e48c3a2ff6dece7dc864f2d42a3ddb7ca3fa6a457f9e8063f9e43ba4b797a"
)
EXPECTED_DATA_SHA256 = (
    "116a0e795c163d6538644baeb874255345e9ced63aec1b81733af4b0c9a8c37e"
)
NONCLAIM = (
    "The 111 uncertified pairs remain open in this package; it does not "
    "exclude m=61, prove fixed r=9, or solve Erdos Problem 617."
)
...
def generate_cores(geng: Path) -> tuple[GraphCase, ...]:
    command = [str(geng), "-q", "-t", "-D8", "16", "61:61"]
    cases = []
...
def minimum_shell_slack(graph: nx.Graph) -> int | None:
    ...
    for epsilon in COMPOSITIONS[budget]:
        if not all(
            degrees[first] + degrees[second] + epsilon[first] + epsilon[second] >= 8
...
def shell_compatible(graph: nx.Graph) -> tuple[bool, int | None]:
    ...
    if any(
        independent(full ^ (1 << omitted), adjacency) for omitted in range(SHELL_ORDER)
    ):
        return False, None
...
def core_demand_profile(core: nx.Graph) -> tuple[int, int]:
    if not nx.is_bipartite(core):
        fail("m=61 core is not bipartite")
    ...
    if sorted(map(len, sides)) != [8, 8]:
        fail("m=61 core does not have an 8+8 bipartition")
    if len(independent_eights(core)) != 2:
        fail("m=61 core does not have exactly two independent eight-sets")
    demand_sums = tuple(
        sum(max(0, core.degree(vertex) - 6) for vertex in side) for side in sides
    )
    ...
    for triangle in itertools.combinations(range(SHELL_ORDER), 3):
        ...
        for core_vertex in range(CORE_ORDER):
            rows.append(
                Inequality(
                    tuple(
                        cross(shell_vertex, core_vertex) for shell_vertex in triangle
                    ),
                    1,
                )
            )
...
def parse_header(raw: Any) -> None:
    ...
    expected = {
        "schema": "erdos617-r9-m61-duals-v1",
...
def main() -> None:
    ...
    print(f"core_structure=K8,8-minus-3 demand_profile={dict(demand_profile)}")
