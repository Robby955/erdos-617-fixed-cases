#!/usr/bin/env python3
"""Generate exact rational duals for strict m=59 covering pairs.

The floating-point optimizer is used only to discover candidate weights.
Every emitted weight vector is normalized and checked with ``Fraction``
before it is written. The separate semantic verifier is the proof-facing
check. This package certifies only the strict-dual subset of the m=59
catalog; it makes no claim about the remaining pairs.
"""
...
NONCLAIM = (
    "The 465 uncertified pairs remain open in this package; it does not "
    "exclude m=59, prove fixed r=9, or solve Erdos Problem 617."
)
...
def generate_cores(geng: Path) -> tuple[GraphCase, ...]:
    command = [str(geng), "-q", "-t", "-D8", "16", "59:59"]
    result = []
...
def main() -> None:
    ...
    core_catalog_sha256 = catalog_sha256(cores)
    shell_catalog_sha256 = catalog_sha256(shells)
    if core_catalog_sha256 != EXPECTED_CORE_CATALOG_SHA256:
        raise AssertionError("core catalog digest mismatch")
    if shell_catalog_sha256 != EXPECTED_SHELL_CATALOG_SHA256:
    ...
    header = {
        ...
        "core_catalog_sha256": core_catalog_sha256,
        "shell_catalog_sha256": shell_catalog_sha256,
        "nonclaim": NONCLAIM,
    }
