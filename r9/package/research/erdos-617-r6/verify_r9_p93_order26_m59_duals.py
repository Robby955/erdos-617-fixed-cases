#!/usr/bin/env python3
"""Semantically verify exact rational duals for strict m=59 pairs.

This verifier reconstructs the 20 cores, the 85 scalar-compatible shells,
and every inequality named by a certificate. It does not call an LP or SAT
solver. It proves only the 1,235 pairs represented by strict exact duals;
the other 465 pairs are outside this package.
"""
...
def core_demand_profile(core: nx.Graph) -> tuple[int, int]:
    ...
    if len(independent_eights(core)) != 2:
        fail("m=59 core does not have exactly two independent eight-sets")
    demand_sums = tuple(
        sum(max(0, core.degree(vertex) - 6) for vertex in side)
        for side in sides
    )
    first, second = sorted(demand_sums)
